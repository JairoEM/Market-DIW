<?php
    ...
    $consuptionID = get_rand_alphanumeric(6);

    // Check if there is any other ID on consuption as the one that we create
    for($i = 0; $i <= 100; $i++){
        try {
            $sqlAux = "SELECT * FROM ID WHERE ID LIKE '$consuptionID';";
    ...
    try {
        $sql = "INSERT INTO consuption VALUES ('$consuptionID','$idBill','$idPlate','green','$hour','$day');";
